...
| Projects | `src/components/Projects.tsx` + `ProjectCard.tsx` | `src/data/projects.ts` | 🟡 2 of 3 real (project-1, project-2); 1 placeholder |
...
`ProjectCard.tsx`'s expanded panel supports the same optional-media pattern for `Project.videos` (`ProjectVideo.src` for local files, or `ProjectVideo.vimeo` for a `VimeoEmbed` — see Architecture above) and `Project.links` (`ProjectLink.url`): a dashed placeholder (video grid slot / disabled link pill) renders until a real value is set, so reserving a slot now and filling it in later needs no layout changes. `Project.images` (`ProjectImage[]`) has no placeholder equivalent — it's additive, only rendered when present (e.g. dashboard/metrics screenshots), in a 2-column grid with each image's optional `caption` as a full paragraph underneath (unlike the short centered captions under videos/photos elsewhere). `project-1` (Starship Solutions AR Activations) is fully real: all 3 links and all 6 Vimeo-hosted AR activation clips (Prize Drop, Solar System, Smash Burger, Cyber Halo, Pop Launch, Rocket Intro). `project-2` (EVE Voice AI & Call Routing System) is fully real: text, tags, and 2 dashboard screenshots (`src/assets/images/eve-metrics/`) with stats captions — no `videos`/`links` (not requested for this one; omit rather than reserving empty slots unless asked).
...
Real content has been filled in for **Hero, About, Timeline, Beyond the Work, `project-1`, and `project-2`** in full (see the table above). **Projects still has 1 of 3 cards on placeholder text** (`project-3`), and **Skills and Contact still hold placeholder text** (generic skill lists, a placeholder email) — these are next up. Content continues to be filled in **section by section**: for sections backed by a `src/data/*.ts` file, replace the placeholder array entries there; for Hero/About/Contact, edit the copy directly in the component.
