...
`ProjectCard.tsx`'s expanded panel supports the same optional-media pattern for `Project.videos` (`ProjectVideo.src`) and `Project.links` (`ProjectLink.url`): a dashed placeholder (video grid slot / disabled link pill) renders until a real value is set, so reserving a slot now and filling it in later needs no layout changes. `project-1` (Starship Solutions AR Activations) has all 3 links filled in (starshipsolutions.ca, its gallery, and the 8th Wall blog post); its 6 video slots are still empty, reserved for AR screen-recordings.
